...
def screenshots(output_dir, image_object):
    ...
    try:
        ...
        cprint(
            f"Error moving screenshots photo from {image_object.path} -> {screenshots_photo_path}: {e}",
            fg.reg,   style.bold,
        )
            ...
            if pic.is_image:
                if JUNK_FILE_REGEX.match(pic.basename):
                    try:
                        ...
                        cprint(
                            f"FileNotFound: Error removing {image_object.path}", fg.red
                        )

                try:--------/mnt/ssd/img/RAW/RAW/ /mnt/ssd/Photos/RAW     /mnt/ssd/img/RAW/RAW/ /mnt/ssd/Photos/RAW     
                    ...
                    if not pic.capture_date:
                        ...
                        if os.path.exists(no_meta_data_image):
                            ...
                            while True:
                                ...
                                try:
                                    new_file_name = (
                                        f"{pic.basename[:-4]}_{count}{pic.extension}"
                                    )
                                    shutil.move(
                                        pic.path,
                        ...
                        # If the file does not exist in 'NoMetaData' directory yet, move it there
                            ...
                            shutil.move(
                                pic.path, os.path.join(output_dir, "NoMetaData", "")
                            )
                    ...
                    # on metadata date
                        ...
                        output_file_path = os.path.join(
                            output_dir, capture_year, pic.basename
                        )
                        ...
                        if os.path.exists(output_file_path):
                            while True:
                                ...
                                try:
                                    ...
                                    output_file_name = (
                                        f"{filename}_{count}{pic.extension}"
                                    )
                                    output_file_path = os.path.join(
                        ...
                        try:
                            os.makedirs(
                                os.path.join(output_dir, capture_year), exist_ok=True
                            )
                            shutil.move(pic.path, output_file_path)
